...
def work_login_init():
    ...
    try:
        url = "http://admin.live-erp.ngx/api/admin/login"
        data = {
        ...
        response_token = res.token
        # print(response_token.json())
        # print(res.text)


        token = ''
        for k, v in response_token.items():
            _token = k + "=" + v + ";"
            # 拿到登录的token内容，token拿到的是字典类型，转换成对应的格式
            token += _token
            # 将登录接口中的token写入缓存中，其中login_token是缓存名称
        CacheHandler.update_cache(cache_name='login_01', value=token)
...
def pytest_collection_modifyitems(items):
    ...
    appoint_items = ["test_login, test_are_staged_add, test_order_list"]
